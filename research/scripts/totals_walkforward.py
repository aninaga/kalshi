"""Monthly walk-forward certification of the totals pace edge.

Prior-season prediction-market data does not exist (Kalshi/Polymarket launched
NBA single-game markets in 2025-26), so the natural certification — more seasons
— is impossible. The strongest test one season allows is a WALK-FORWARD: the
strategy direction (`continuation`) is mechanism-pre-registered, NOT fit, so
EVERY game is a valid out-of-sample observation. We then ask: does the edge
reappear, independently, month after month? A real edge is stable across months;
an artifact is concentrated in one window. This directly attacks the gate's
season-split stability concern with finer granularity and the full game count.

It also runs the full-population gate (block-bootstrap-by-game over ALL games),
which — with ~1,300 games instead of 199 — is the tightest CI the data permits.

Usage::

    python3 -m research.scripts.totals_walkforward --thresh 6 --max-stale-min 2
"""
...
from research.scripts.totals_alpha import build_trades, evaluate  # noqa: E402
...
def main():
    ...
    # Pre-registered direction = continuation (pace under-reaction -> over).
    ...
    print("=== FULL POPULATION (all games, direction pre-registered) ===")
    ...
    if zc["reasons"]:
        print(f"  gate@{a.cost:.0f}c reasons:", "; ".join(zc["reasons"][:4]), flush=True)

    print(f"\n=== MONTHLY WALK-FORWARD (each month is independent OOS, cost={a.cost:.0f}c) ===")
