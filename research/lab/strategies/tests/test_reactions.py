...
def _install_strategy_stub() -> None:
    # Post-integration the real research.lab.strategy exists — prefer it and do
    # NOT clobber it (a module-level stub would leak across the whole test run).
    try:
        import research.lab.strategy  # noqa: F401
        return
    except ImportError:
        pass
...
def test_registry_constructs_every_strategy() -> None:
    # Post-integration the factories build the REAL lab.strategy.Strategy; in a
    # bare worktree they build the stub. Assert duck-typed shape, not the class.
    for fac in reactions.REACTIONS.values():
        strat = fac()
        assert hasattr(strat, "name") and isinstance(strat.name, str)
